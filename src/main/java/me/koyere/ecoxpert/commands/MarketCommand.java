...
public class MarketCommand implements TabExecutor {
    ...
    public MarketCommand(MarketManager marketManager, TranslationManager translationManager, Logger logger) {
        ...
        this.ordersGUI = new me.koyere.ecoxpert.modules.market.orders.MarketOrdersGUI(orderService, translationManager,
                logger);
    }
    ...
    public me.koyere.ecoxpert.modules.market.orders.MarketOrdersGUI getOrdersGUI() {
        return ordersGUI;
    }
    ...
    private boolean handleOrderListCreate(Player player, String[] args) {
        ...
        try {
            unit = new java.math.BigDecimal(args[3]);
            if (unit.signum() <= 0)
                throw new NumberFormatException();
        } catch (Exception e) {
            player.sendMessage(translationManager.getMessage("market.order.invalid"));
            return true;
        }
        if (args.length >= 5) {
            try {
        ...
        orderService.createListing(player, mat, qty, unit, hours)
    ...
    private boolean handleOrderListShow(Player player, String[] args) {
        ...
        if (args.length >= 2) {
            try {
                filter = Material.valueOf(args[1].toUpperCase());
            } catch (Exception ignored) {
            }
        }
    ...
    private boolean handleOrderBuy(Player player, String[] args) {
        ...
        orderService.buyFromOrder(player, id, qty)
                .thenAccept(msg -> player.sendMessage(translationManager.getMessage("prefix") + msg));
        return true;
    }
    ...
    private boolean handleBuyCommand(Player player, String[] args) {
        ...
        CompletableFuture<MarketTransactionResult> buyFuture = marketManager.buyItem(player, material, quantity);
    ...
    private boolean handleSellCommand(Player player, String[] args) {
        ...
        // Support UX: /market sell <amount> uses item in hand and sells max quantity to
        // reach ~amount
        ...
        if (targetAmount != null) {
            material = player.getInventory().getItemInMainHand() != null
        ...
        if (targetAmount != null) {
            ...
            quantity = targetAmount.divide(price, 0, RoundingMode.DOWN).intValue();
            if (quantity <= 0)
                quantity = 1;
        ...
        CompletableFuture<MarketTransactionResult> sellFuture = marketManager.sellItem(player, material, quantity);
    ...
    private void showItemPrices(Player player, Material material) {
        ...
        itemFuture.whenComplete((itemOpt, throwable) -> {
            ...
            if (item.isSellable()) {
                ...
                java.math.BigDecimal eff2 = item.getCurrentSellPrice().multiply(java.math.BigDecimal.valueOf(f2))
                        .setScale(2, java.math.RoundingMode.HALF_UP);
                ...
                for (String key : section.getKeys(false)) {
                    ...
                    for (String m : mats) {
                        if (material.name().equalsIgnoreCase(m)) {
                            String ck = "roles." + role + ".category_bonuses." + key.toLowerCase() + "."
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            // Main subcommands
            List<String> subcommands = Arrays.asList("buy", "sell", "prices", "stats", "orders", "list", "buyorder",
                    "help");
