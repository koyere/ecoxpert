...
public class VaultEconomyProviderImpl implements VaultEconomyProvider, Economy {
    ...
    /**
     * Check if player has an account (by name)
     * Note: Uses deprecated getOfflinePlayer(String) as required by Vault API
     */
    ...
    /**
     * Withdraw from player account (by name)
     * Note: Uses deprecated getOfflinePlayer(String) as required by Vault API
     */
    @SuppressWarnings("deprecation")
    @Override
    ...
    /**
     * Deposit to player account (by name)
     * Note: Uses deprecated getOfflinePlayer(String) as required by Vault API
     */
    @SuppressWarnings("deprecation")
    @Override
    ...
    // Currently, EcoXpert only supports personal player bank accounts via
    // BankManager.
    //
    // FUTURE IMPLEMENTATION NOTES:
    // - Shared banks could be used for Factions, Guilds, Towns (Towny), Lands, etc.
    // - Database schema will need: ecoxpert_shared_banks (id, name, balance,
    ...
    public EconomyResponse deleteBank(String name) {
        ...
        // Implementation: Soft delete or hard delete with balance redistribution to
        // owner
    ...
    public EconomyResponse isBankMember(String name, String playerName) {
        ...
        // Implementation: SELECT FROM ecoxpert_shared_bank_members WHERE bank_name = ?
        // AND player_uuid = ?
        return new EconomyResponse(0, 0, EconomyResponse.ResponseType.NOT_IMPLEMENTED,
                "Shared bank accounts not yet implemented");
    }
        ...
        try {
            if (!hasAccount(player)) {
                economyManager.createAccount(
                        player.getUniqueId(),
                        economyManager.getStartingBalance()).join();
            }
            return true;
        } catch (CompletionException e) {
