 ...
 * creates realistic economic scenarios and responds intelligently to server
 * conditions.
 * 
 * Unlike static economy plugins, this system creates DYNAMIC ECONOMIC EVENTS
 * that
 * simulate real-world economic phenomena and keep the economy engaging and
 * balanced.
 * 
...
public class EconomicEventEngine {
    ...
    private double calculateEventProbability(EconomicIntelligenceEngine.EconomicCycle cycle,
        ...
        if (health < 0.3)
            baseProbability += 0.3; // Crisis events
        if (health > 0.9)
            baseProbability += 0.2; // Celebration events

        // Inflation-based adjustments
        if (Math.abs(inflation) > 0.05)
            baseProbability += 0.2; // Extreme inflation events
        ...
        if (hoursSinceLastEvent < 2)
            baseProbability *= 0.1; // Drastically reduce if recent event
        ...
        switch (cycle) {
            case DEPRESSION -> {
                ...
                if (type == EconomicEventType.TRADE_BOOM)
                    modifier *= 1.5;
                if (type == EconomicEventType.MARKET_CORRECTION)
                    modifier *= 0.7;
            }
            case RECESSION -> {
                if (type == EconomicEventType.TRADE_BOOM)
            ...
            case GROWTH -> {
                if (type == EconomicEventType.INVESTMENT_OPPORTUNITY
                        || type == EconomicEventType.TECHNOLOGICAL_BREAKTHROUGH)
                    modifier *= 1.5;
            }
            case BOOM -> {
    ...
    public boolean triggerEvent(EconomicEventType eventType) {
        try {
            ...
            if (lastEnd != null
                    && java.time.Duration.between(lastEnd, java.time.LocalDateTime.now()).toHours() < cooldownHours) {
    ...
    private boolean isTypeActive(EconomicEventType type) {
        for (EconomicEvent ev : activeEvents.values()) {
            if (ev.getType() == type && ev.getStatus() == EconomicEvent.EventStatus.ACTIVE)
                return true;
        }
        return false;
    }
    ...
    public int getActiveEventsCount() {
        int c = 0;
        for (EconomicEvent ev : activeEvents.values()) {
            if (ev.getStatus() == EconomicEvent.EventStatus.ACTIVE)
                c++;
        }
        return c;
    }
    ...
    /**
     * Get remaining cooldown hours for the event type based on the last end time
     * map.
    ...
    private void persistEvent(EconomicEvent event, String status) {
        try {
            ...
            dm.executeUpdate(
                    ...
                    event.getId(), event.getType().name(), status, paramsJson).join();
        } catch (Exception ignored) {
        }
    }
    ...
    private void persistEventEnd(EconomicEvent event) {
        try {
            ...
            dm.executeUpdate(
                    ...
                    "COMPLETED", paramsJson, event.getId()).join();
        } catch (Exception ignored) {
        }
    }

    // Minimal JSON serialization for parameters (numbers, strings, lists of
    // Material)
    ...
    private void startEvent(EconomicEvent event) {
        ...
        try {
            me.koyere.ecoxpert.core.translation.TranslationManager tm = plugin.getServiceRegistry()
    ...
    private void applyGovernmentStimulus(EconomicEvent event) {
        ...
        Bukkit.getOnlinePlayers().forEach(player -> {
            ...
            player.sendMessage(
                    tm.getMessage("prefix") + tm.getPlayerMessage(player, "events.stimulus.received", amountStr));
        });
    ...
    private void applyMarketDiscovery(EconomicEvent event) {
        ...
        Bukkit.broadcastMessage(
                tm.getMessage("prefix") + tm.getMessage("events.market_discovery.announce", itemList.toString(), pct));
        ...
        // Apply temporary per-item sell boost (encourage players to supply discovered
        // items)
    ...
    private int calculateIntelligentDuration(int baseDuration) {
        ...
        if (inflationManager != null && inflationManager.isActive()) {
            ...
            if (economicHealth < 0.4)
                multiplier = 0.7; // Shorter during crisis
            else if (economicHealth > 0.8)
                multiplier = 1.3; // Longer during prosperity
        }
    ...
    private double calculateIntelligentIntensity(double baseIntensity) {
        ...
        if (playerCount > 20)
            intensity *= 1.2; // More intense for larger servers
        else if (playerCount < 5)
            intensity *= 0.8; // Less intense for smaller servers
    ...
    private Map<String, Object> createSmartEventParameters(EconomicEventType eventType,
        ...
        switch (eventType) {
            ...
            // Other event types (SEASONAL_DEMAND, BLACK_SWAN_EVENT, INVESTMENT_OPPORTUNITY,
            // RESOURCE_SHORTAGE, LUXURY_DEMAND, MARKET_CORRECTION,
            // TECHNOLOGICAL_BREAKTHROUGH)
            // have their parameters defined elsewhere in the effect implementation methods
            default -> {
            }
        }
    ...
    private void applyInvestmentOpportunity(EconomicEvent event) {
        if (marketManager == null)
            return;
        try {
            me.koyere.ecoxpert.core.config.ConfigManager cfg = plugin.getServiceRegistry()
            ...
            if (event.getAffectedItems() != null && !event.getAffectedItems().isEmpty()) {
                ...
                for (String s : new String[] { "EMERALD", "DIAMOND", "GOLD_INGOT" }) {
                    try {
                        targets.add(Material.valueOf(s));
    ...
    private void applyMarketCorrection(EconomicEvent event) {
        // Apply a temporary global cooling: raise buy prices a bit, lower sell prices a
        // bit
        try {
            me.koyere.ecoxpert.core.config.ConfigManager cfg = plugin.getServiceRegistry()
                    .getInstance(me.koyere.ecoxpert.core.config.ConfigManager.class);
            ...
            double sellDelta = evcfg.getDouble("market_correction.global_sell_factor_delta", -0.02); // -2% returns to
                                                                                                     // sell
            ...
            Bukkit.broadcastMessage(
                    tm.getMessage("prefix") + tm.getMessage("events.market_correction.announce", buyPct, sellPct));

            // Reset after duration
            Bukkit.getScheduler().runTaskLater(plugin, () -> marketManager.setGlobalPriceFactors(prev[0], prev[1]),
                    minutes * 60L * 20L);
    ...
    private void applyResourceShortage(EconomicEvent event) {
        if (marketManager == null || event.getAffectedItems() == null || event.getAffectedItems().isEmpty())
            return;
        try {
            me.koyere.ecoxpert.core.config.ConfigManager cfg = plugin.getServiceRegistry()
                    .getInstance(me.koyere.ecoxpert.core.config.ConfigManager.class);
    ...
    private void applyTechnologicalBreakthrough(EconomicEvent event) {
        ...
        try {
            ...
            double sellDelta = evcfg.getDouble("technological_breakthrough.sell_delta", -0.02); // slightly lower sell
                                                                                                // (deflation)
            java.util.Map<Material, double[]> factors = new java.util.HashMap<>();
            for (Material m : targets) {
                factors.put(m, new double[] { 1.0 + buyDelta, 1.0 + sellDelta });
            ...
            String sellPct = String.format("%.0f", Math.abs(sellDelta) * 100);
            Bukkit.broadcastMessage(
                    tm.getMessage("prefix") + tm.getMessage("events.tech_breakthrough.announce", buyPct, sellPct));
