...
public class MarketOrderServiceImpl implements MarketOrderService {
    ...
    public CompletableFuture<String> createListing(Player seller, Material material, int quantity, BigDecimal unitPrice, int expiryHours) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            Bukkit.getScheduler().runTask(plugin, () -> marketManager.removeItemsFromInventory(seller, material, quantity));
            ...
            dataManager.executeUpdate(sql, seller.getUniqueId().toString(), material.name(), unitPrice.setScale(2, RoundingMode.HALF_UP), quantity, expiryHours).join();
            // Fetch last id
            long id = 0;
            try (QueryResult qr = dataManager.executeQuery("SELECT last_insert_rowid() as id").join()) {
                if (qr.next()) id = qr.getLong("id");
            } catch (Exception ignored) {}
            return tm.getMessage("market.order.created", quantity, material.name(), economyManager.formatMoney(unitPrice), id);
        });
    }
    ...
    public CompletableFuture<String> buyFromOrder(Player buyer, long orderId, int quantity) {
        return CompletableFuture.supplyAsync(() -> {
            try (QueryResult qr = dataManager.executeQuery(
                ...
                orderId).join()) {
                if (!qr.next()) return tm.getMessage("market.order.not-found", orderId);
                if (!"OPEN".equalsIgnoreCase(qr.getString("status"))) return tm.getMessage("market.order.closed", orderId);
                ...
                if (!okFunds) return tm.getMessage("market.error.insufficient-funds", economyManager.formatMoney(total), "");
                // Transfer funds
                boolean ok = economyManager.transferMoney(buyer.getUniqueId(), seller, total, "OrderBook purchase").join();
                if (!ok) return tm.getMessage("market.error.system-error");
                // Give items
                ItemStack stack = new ItemStack(mat, quantity);
                boolean added = marketManager.addItemsToInventory(buyer, stack);
                if (!added) return tm.getMessage("market.error.inventory-full");
                // Update order remaining
                int newRem = remaining - quantity;
                String status = newRem <= 0 ? "CLOSED" : "OPEN";
                dataManager.executeUpdate("UPDATE ecoxpert_market_orders SET remaining_quantity = ?, status = ? WHERE id = ?",
                    newRem, status, orderId).join();
                return tm.getMessage("market.order.buy.success", quantity, mat.name(), economyManager.formatMoney(total), orderId);
            } catch (Exception e) {
    ...
    public CompletableFuture<List<MarketOrder>> listOpenOrders(Material filter) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (QueryResult qr = filter == null ?
                dataManager.executeQuery(base).join() :
                ...
                while (qr.next()) {
                    out.add(new MarketOrder(
                        ...
                        qr.getTimestamp("expires_at") != null ? qr.getTimestamp("expires_at").toLocalDateTime() : null
                    ));
                }
            } catch (Exception ignored) {}
            return out;
