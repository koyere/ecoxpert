...
public final class MarketStatistics {
    ...
    public BigDecimal getAverageTransactionValue() {
        ...
        return totalVolume.divide(BigDecimal.valueOf(totalTransactions), 2, BigDecimal.ROUND_HALF_UP);
    }
    ...
    public String getActivityLevel() {
        if (marketActivity >= 0.8) return "Very High";
        if (marketActivity >= 0.6) return "High";
        if (marketActivity >= 0.4) return "Medium";
        if (marketActivity >= 0.2) return "Low";
        return "Very Low";
    }
