...
public class PlayerEconomicProfile {
    ...
    private void updatePersonalityClassification() {
        ...
        if (maxScore == saverScore)
            personality = EconomicPersonality.SAVER;
        else if (maxScore == traderScore)
            personality = EconomicPersonality.TRADER;
        else if (maxScore == spenderScore)
            personality = EconomicPersonality.SPENDER;
        else if (maxScore == investorScore)
            personality = EconomicPersonality.INVESTOR;
        else if (maxScore == speculatorScore)
            personality = EconomicPersonality.SPECULATOR;
        else if (maxScore == hoarderScore)
            personality = EconomicPersonality.HOARDER;
    ...
    private double calculateTrustScore() {
        ...
        double longevityScore = Math.min(1.0,
                java.time.Duration.between(createdAt, LocalDateTime.now()).toDays() / 365.0);
    ...
    public List<String> getRecommendedActions() {
        ...
        if (creditScore < 600) {
            recommendations
                    .add("Improve your credit score by making regular transactions and contributing to the economy");
        }
