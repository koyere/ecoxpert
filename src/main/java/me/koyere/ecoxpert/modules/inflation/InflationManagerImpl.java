...
public class InflationManagerImpl implements InflationManager {
        ...
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                try {
                    ...
                    boolean edu = cfg.getBoolean("education.enabled", true)
                            && cfg.getBoolean("education.broadcasts.cycle", true);
                    if (edu) {
                            ...
                            try {
                                ...
                                if (current != lastBroadcastCycle) {
                                    ...
                                    me.koyere.ecoxpert.core.education.EducationNotifier.broadcastCycle(plugin,
                                            plugin.getServiceRegistry().getInstance(
                                                    me.koyere.ecoxpert.core.translation.TranslationManager.class),
                                            current);
                                    // Fire API event on main thread
                                    org.bukkit.Bukkit.getScheduler().runTask(plugin,
                                            () -> org.bukkit.Bukkit.getPluginManager().callEvent(
                                                    new me.koyere.ecoxpert.api.events.EconomyCycleChangeEvent(old,
    ...
    public EconomicIntelligenceEngine.EconomicCycle getCurrentCycle() {
        return active && intelligenceEngine != null ? intelligenceEngine.getCurrentCycle()
                : EconomicIntelligenceEngine.EconomicCycle.STABLE;
    }

    @Override
    public double getEconomicHealth() {
        return active && intelligenceEngine != null ? intelligenceEngine.getEconomicHealth() : 1.0;
    }

    @Override
    public double getInflationRate() {
        return active && intelligenceEngine != null ? intelligenceEngine.getInflationRate() : 0.0;
    }

    @Override
    public double getVelocityOfMoney() {
        return active && intelligenceEngine != null ? intelligenceEngine.getVelocityOfMoney() : 1.0;
    }
    ...
    public EconomicMemory.EconomicForecast getEconomicForecast() {
        return active && economicMemory != null ? economicMemory.getForecast() : null;
    }
    ...
    public List<EconomicMemory.EconomicAnomaly> detectAnomalies() {
        ...
        return getCurrentSnapshot().thenApply(snapshot -> economicMemory.detectAnomalies(snapshot)).join();
    }
    ...
    public CompletableFuture<Void> forceIntervention(EconomicSnapshot.InterventionType type, double magnitude) {
        ...
        return CompletableFuture.runAsync(() -> {
            plugin.getLogger()
                    .info("🚨 Admin forced economic intervention: " + type + " (magnitude: " + magnitude + ")");
    ...
    public double getInterventionEffectiveness(EconomicSnapshot.InterventionType type) {
        return active && economicMemory != null ? economicMemory.getInterventionEffectiveness(type) : 0.5;
    }
    ...
    public String getEconomicStatistics() {
        ...
        if (forecast != null) {
            ...
            stats.append("Predicted Health: ").append(String.format("%.1f%%", forecast.getPredictedHealth() * 100))
                    .append("\n");
            stats.append("Predicted Inflation: ")
                    .append(String.format("%.2f%%", forecast.getPredictedInflation() * 100)).append("\n");
            stats.append("Forecast Confidence: ").append(String.format("%.1f%%", forecast.getConfidence() * 100))
                    .append("\n");
        }
    ...
    private void applyIntervention(EconomicSnapshot.InterventionType type, double magnitude, String reason) {
        switch (type) {
            ...
            // NONE means no intervention needed
            default -> {
            }
        }
