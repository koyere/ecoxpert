...
public class EconomicMemory {
    ...
    public List<EconomicAnomaly> detectAnomalies(EconomicSnapshot currentSnapshot) {
        ...
        if (Math.abs(currentSnapshot.getInflationRate()) > 0.1) {
            anomalies.add(new EconomicAnomaly(
                    ...
                    "Extreme inflation rate detected: "
                            + String.format("%.2f%%", currentSnapshot.getInflationRate() * 100),
                    0.9));
        }
    ...
    private double calculateTrend(List<EconomicSnapshot> snapshots,
            java.util.function.Function<EconomicSnapshot, Double> valueExtractor) {
    ...
    private void updatePatternRecognition() {
        ...
        if (snapshots.size() < 48)
            return; // Need at least 4 hours of data
