...
public class BankGUI extends BaseGUI {
    ...
    public BankGUI(EcoXpertPlugin plugin, BankManager bankManager, EconomyManager economyManager,
    ...
    public void open(Player player) {
        ...
        if (platformManager.isBedrockPlayer(player) && bedrockFormsManager != null
                && bedrockFormsManager.isFormsAvailable()) {
    ...
    private void openBedrockMainMenu(Player player) {
        ...
        bankManager.getBalance(player.getUniqueId()).thenAccept(balance -> {
            ...
            content.append("§7").append(tm.getMessage("bank.gui.bedrock.welcome", "Welcome to the Bank!"))
                    .append("\n\n");
    ...
    private void openDepositForm(Player player) {
        economyManager.getBalance(player.getUniqueId()).thenAccept(walletBalance -> {
            StringBuilder content = new StringBuilder();
            content.append("§7").append(tm.getMessage("bank.gui.bedrock.deposit.info", "Select amount to deposit:"))
                    .append("\n\n");
            content.append("§7").append(tm.getMessage("bank.gui.bedrock.wallet", "Wallet")).append(": §e")
                    .append(economyManager.formatMoney(walletBalance));
    ...
    private void openWithdrawForm(Player player) {
        bankManager.getBalance(player.getUniqueId()).thenAccept(bankBalance -> {
            StringBuilder content = new StringBuilder();
            content.append("§7").append(tm.getMessage("bank.gui.bedrock.withdraw.info", "Select amount to withdraw:"))
                    .append("\n\n");
            content.append("§7").append(tm.getMessage("bank.gui.bedrock.balance", "Bank Balance")).append(": §e")
    ...
    protected Inventory create(Player player) {
        ...
        inv.setItem(11, button(Material.LIME_WOOL, tm.getMessage("bank.gui.deposit") + " +100",
                new String[] { "§7" + tm.getMessage("bank.gui.deposit-lore1"),
                        "§7" + tm.getMessage("bank.gui.deposit-lore2") }));
        inv.setItem(12, button(Material.LIME_WOOL, tm.getMessage("bank.gui.deposit") + " +1000",
                new String[] { "§7" + tm.getMessage("bank.gui.deposit-fast-lore1"),
                        "§7" + tm.getMessage("bank.gui.deposit-fast-lore2") }));
        ...
        inv.setItem(14, button(Material.RED_WOOL, tm.getMessage("bank.gui.withdraw") + " -100",
                new String[] { "§7" + tm.getMessage("bank.gui.withdraw-lore1"),
                        "§7" + tm.getMessage("bank.gui.withdraw-lore2") }));
        inv.setItem(15, button(Material.RED_WOOL, tm.getMessage("bank.gui.withdraw") + " -1000",
                new String[] { "§7" + tm.getMessage("bank.gui.withdraw-fast-lore1"),
                        "§7" + tm.getMessage("bank.gui.withdraw-fast-lore2") }));
        return inv;
    }
    ...
    // Overloaded helper method called by create() - IDE false positive about not
    // being used
    ...
    protected void onClick(InventoryClickEvent e) {
        ...
        if (name.contains("+1000")) {
            ...
            bankManager.getBalance(p.getUniqueId()).thenAccept(b -> p.sendMessage(
                    tm.getMessage("prefix") + tm.getMessage("bank.balance", economyManager.formatMoney(b))));
        }
    }
    ...
    private void sendResult(Player player, me.koyere.ecoxpert.modules.bank.BankOperationResult result) {
        if (result.isSuccess()) {
            ...
            if (result.getNewBalance() != null && result.getAmount() != null) {
                String key = result.getMessage().toLowerCase().contains("withdraw") ? "bank.withdraw.success"
