...
public class PlaceholderProvider extends PlaceholderExpansion {
    ...
    private static final Pattern BALTOP_BALANCE_PATTERN = Pattern.compile("(?:essentials_)?baltop_balance(?:_(commas|fixed|formatted))?_(\\d+)");
    private static final Pattern BALTOP_PLAYER_PATTERN = Pattern.compile("(?:essentials_)?baltop_player(?:_stripped)?_(\\d+)");
        ...
        try {
            ...
            switch (key) {
                ...
                case "bank_daily_deposit_remaining": {
                    if (player == null || bankManager == null) return "0";
                    BigDecimal remaining = bankManager.getDailyLimitRemaining(player.getUniqueId(), BankTransactionType.DEPOSIT).join();
                    return formatMoneySafe(remaining);
                }
                case "bank_daily_withdraw_remaining": {
                    if (player == null || bankManager == null) return "0";
                    BigDecimal remaining = bankManager.getDailyLimitRemaining(player.getUniqueId(), BankTransactionType.WITHDRAW).join();
                    return formatMoneySafe(remaining);
                }
                case "bank_daily_transfer_remaining": {
                    if (player == null || bankManager == null) return "0";
                    BigDecimal remaining = bankManager.getDailyLimitRemaining(player.getUniqueId(), BankTransactionType.TRANSFER_OUT).join();
                    return formatMoneySafe(remaining);
                }
                case "loan_status": {
                ...
                case "loan_next_due_in_days": {
                    if (player == null) return "0";
                    var summary = getLoanScheduleSummary(player.getUniqueId());
                    if (summary.nextDate() == null) return "0";
