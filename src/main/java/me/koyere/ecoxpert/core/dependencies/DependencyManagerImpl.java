...
public class DependencyManagerImpl implements DependencyManager {
    ...
    private DependencyResult downloadAndVerifyDependency(DependencyInfo info) {
        ...
        try {
            ...
            for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    plugin.getLogger().info("Downloading " + dependencyName + " (attempt " + attempt + "/" + MAX_RETRIES + ")");
    ...
    private CompletableFuture<DependencyResult> attemptFallback(DependencyInfo info) {
        ...
        if (info.isRequired()) {
            return CompletableFuture.completedFuture(
                DependencyResult.failure(DependencyState.FAILED, "Required dependency unavailable and no fallback")
            );
        } else {
            dependencyStates.put(dependencyName, DependencyState.FALLBACK);
            return CompletableFuture.completedFuture(
    ...
    private void runHealthCheck() {
        try {
            ...
            if (!healthy) {
                plugin.getLogger().warning("Dependency system is unhealthy - some required dependencies are unavailable");
            }
