        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                String summary = corruptionDetected
                        ? (repairSuccessful ? "Corruption detected and repaired"
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            if (checkpoints.size() > 100) {
                ...
                checkpoints.entrySet()
                        .removeIf(entry -> entry.getValue().timestamp.isBefore(Instant.now().minusSeconds(3600)));
            }
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(transactionLogFile.toFile(), true))) {
            ...
            while ((transaction = transactionQueue.poll()) != null) {
                ...
                writer.println(String.format(
                        "{\"id\":\"%s\",\"timestamp\":\"%s\",\"type\":\"%s\",\"amount\":\"%s\",\"success\":%b}",
